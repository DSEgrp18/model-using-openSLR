...
Designed for Google Colab GPU runtimes. Example:

  python scripts/train_vits.py \\
    --dataset-path /content/drive/MyDrive/sinhala-vits/dataset \\
    --output-path /content/drive/MyDrive/sinhala-vits/outputs \\
    --epochs 1000 \\
    --batch-size 16
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional Coqui checkpoint to resume/finetune from",
    )
    parser.add_argument(
        "--continue-path",
        type=Path,
        default=None,
        help="Existing Trainer run directory to continue",
    )
...
def main() -> int:
    ...
    # Import after path setup so the custom formatter is available.
    ...
    try:
        ...
        print(
            "error: Coqui TTS is not installed. In Colab run:\n"
    ...
    model_args = VitsArgs(
        ...
        num_speakers=0,  # filled after speaker manager init
    ...
    config = VitsConfig(
        ...
        test_sentences=[
            ["ආයුබෝවන්", "sin_2241"],
            ["මම සිංහලෙන් කතා කරනවා", "sin_4191"],
            ["ඔබට සුභ දවසක් වේවා", "sin_6314"],
        ],
    )

    # Register custom formatter with Coqui (name varies across TTS versions).
...
if __name__ == "__main__":
    # Avoid tokenizer parallelism warnings in forked loaders.
